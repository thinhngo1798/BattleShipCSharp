...
static class DeploymentController
{
    ...
    private static ShipName GetShipMouseIsOverTest(Point2D pt)
    {
        foreach (ShipName sn in Enum.GetValues(typeof(ShipName)))
        {
            ...
            if (UtilityFunctions.IsMouseInRectangleTest(SHIPS_LEFT, SHIPS_TOP + i * SHIPS_HEIGHT, SHIPS_WIDTH, SHIPS_HEIGHT,pt))
            {
                return sn;
            }
        }
    ...
    {
        [Test]
        public void CheckingChoosingShipsInSettingGame()
        {
